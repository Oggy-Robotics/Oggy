...
namespace Ogame_Robot.Clases.Military
{
    public abstract class UnitInfo
    {
        ...
        public abstract int GetBaseSpeed( PlayerMilitary player );
        public abstract int GetActualSpeed( PlayerMilitary player );
        public abstract int GetFuelConsumption( PlayerMilitary player );


        public Unit[] MakeUnits(PlayerMilitary player, int count ) {
            int actualArmor = GetHull(player.ArmorTech);
            int actualShield = GetShield(player.ShieldTech);
            int actualWeapons = GetWeapons(player.WeaponTech);
            Unit[] units = new Unit[count];
            for( int i = 0; i < count; i++ ) {
                units[i].Armor = actualArmor;
                units[i].Shield = actualShield;
                units[i].Weapons = actualWeapons;
            }
            return units;
        }
        ...
        public static readonly Dictionary<UnitType, UnitInfo> Factories = new Dictionary<UnitType, UnitInfo>();
